...
### Durante Fases 0–2 (BUILD)
As 6 tasks EDGE permanecem rodando no cloud Celery **sem alteração**.
Recognition está em estado pre-produção (zero clientes ativos). RVB será o
primeiro tenant, entrando junto com o edge deployment.
Novos serviços em `services/inference/` são desenvolvidos em paralelo mas NÃO recebem tráfego.

### Fase 3 (CUTOVER DIRETO)
As 6 tasks EDGE migram do monolito `api-v3` para `services/inference/` com
cutover direto — sem período de coexistência (shadow mode).
`INFERENCE_ENGINE` selecionável por env var: `deepstream` para edge, `ultralytics` como fallback cloud.

### Pós-RVB Go-Live (FORA DO PLANO ATUAL)
Após go-live estável: tasks Celery EDGE removidas do monolito em sprint dedicada.

**Referência:** ADR-0013 — docs/decisions/adr/0013-direct-cutover-no-shadow.md
