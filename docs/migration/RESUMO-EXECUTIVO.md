...
1. **O tempo real não funciona no servidor hoje — e quando funcionar, vaza entre tenants.** Com as versões pinadas em produção (`python-socketio 5.16.3` / `flask-socketio 5.6.1`), `create_app()` não registra handler nem `namespaces=`; **reproduzi localmente**: `/` conecta, `/monitor`, `/training`, `/quality`, `/admin` são recusados (`server.namespaces=['/']`). Todas as telas funcionam por polling sem saber. Além disso o bridge emite em broadcast sem `join_room(tenant)` (C-01). **Consequência:** o novo front não pode ser desenhado "WebSocket-first" até o backend (a) registrar `connect` por namespace com JWT e rooms por tenant, (b) alinhar os shapes (5 divergências listadas em `socketio-env.md` A5). Polling continua sendo o contrato real.
...
2. Backend abre 3 frentes **antes** de qualquer corte: (a) SocketIO — handlers de namespace + JWT + rooms por tenant; (b) fila `risk:security` acima; (c) depreciação explícita das duplicatas (`/api` legado, branding, retention, treino por `user_id`).
