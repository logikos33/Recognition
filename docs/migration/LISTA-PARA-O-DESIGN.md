...
_GAP-DE-PRODUTO neste domínio: 3 endpoint(s) sem UI — ver anexo A.4._
...
_GAP-DE-PRODUTO neste domínio: 18 endpoint(s) sem UI — ver anexo A.8._
...
9. Dashboard Integrado: telemetria ao vivo deve chegar filtrada por tenant (exigir correção do backend/socket antes de expor a mais de um tenant) e os empty-states devem refletir produtores reais (heartbeat do box), não scripts inexistentes.
...
4. Fila de verificação humana com imagem da evidência por item (hoje a fila não traz a imagem — usar o snapshot do alerta), motivo da triagem automática, ações Confirmar/Rejeitar e badge de pendências na navegação.
5. Feedback do operador em qualquer evento/alerta ('detecção correta / errada / incerta' + classe corrigida) e painel 'qualidade do modelo' com resumo por módulo e veredito — fecha o flywheel de treino que o backend já persiste.
...
### A.4 Admin auxiliar — branding, integrações, versões, observabilidade, consoles de teste/demo (3)

| Método | Path | O que o back oferece (resposta) | Por que é gap |
|---|---|---|---|
| GET | `/api/v1/admin/branding/tenants` | data.tenants[]: {id, name, slug, is_active, branding:{JSONB cru}} | lista em lote de branding por tenant que a tela White-label deveria usar — hoje AdminBrandingTenantsPage faz N+1 (GET /admin/tenants + GET /admin/tenants/<id>/branding por tenant) |
| GET | `/api/v1/admin/introspection` | data: {ru_maxrss, ru_maxrss_mb, rss_current_mb\|null, uptime_seconds, requests_served (por worker), storage_backend:'r2'\|'local'\|'desconhecido', worker_class, service_type, live_view:{segments_buffered, bytes_buffered, avg_segment_bytes, streams_active, degraded}} | diagnóstico do processo (platô × vazamento de memória) consumido só manualmente via curl; candidato a card 'Processo' na aba Infra da Observabilidade |
...
| GET | `/api/training/jobs/<job_id>/status` | data: linha completa de training_jobs (inclui callback_token) | Detalhe de um job do histórico (deep-link/drawer) — wrapper trainingService.getJobStatus existe mas nenhuma tela chama. |

### A.7 Modelos (rollout), datasets, cenários, módulos e regras (15)
...
| GET | `/api/v1/datasets` | data: {datasets[]: {id, tenant_id, module_code, name, description, created_by, created_at}, total} | Pipeline de treino (WS-A3) — datasets pai + versões COCO; front atual não tem tela de datasets (TrainingPage cria job sem dataset_version_id; versões hoje são criadas por script/cURL). |
...
| GET | `/api/v1/models/<model_id>/eval` | data.evaluation: {id, tenant_id, model_id, champion_model_id, dataset_version_id, metrics, confusion_matrix, verdict:'promote'\|'reject'\|..., created_at} | Resultado da avaliação campeão×desafiante (é o que gera o 409 do activate) sem UI — o usuário vê 'reprovado' sem conseguir ver por quê. |
...
### A.8 Módulo Qualidade (inspeções, gate, estações, relatórios, treino) (18)
...
| GET | `/api/verification/queue/count` | data: {count} | Badge de pendências na navegação ('Verificação') — o sidebar atual não mostra contagem |
