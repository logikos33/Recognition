...
Two steps per command: `RecorderClient.get_snapshot(camera_id)` (ONVIF
GetSnapshotUri, D-85, falling back to a live-frame RTSP grab — see
onvif_recorder_client.py/rtsp_timestamp_recorder_client.py) then a multipart
POST of the JPEG to the cloud (`POST /api/v1/edge/cameras/<camera_id>/snapshot`,
device auth, mirrors live_view/segment_pusher.py's push pattern).
...
from .recorder_client import RecorderAuthError, RecorderError
...
class SnapshotExecutor:
    ...
    *http_client* follows the same convention as command_poller.py/
    ...
    def capture_and_upload(self, camera_id: str, channel: Any = None) -> dict:
        ...
        *channel* is accepted (from the command payload) but NOT used here —
        the RecorderClient resolves camera_id -> channel itself via its own
        channel_map (same source of truth config_poller/recorder_factory
        already maintain), so a stale/wrong channel in an old queued command
        payload can never desync from the box's live mapping.
        """
