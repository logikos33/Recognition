 ...
 *
 * PR-B (30/08) demoliu `EpiAlerts.tsx`/`AlertDetailPage.tsx`; o FLIP (29/08,
 * mergeado depois) tirou o prefixo `/novo`. Juntos: `/epi/alerts` hoje
 * REDIRECIONA (client-side, `AppRoutes.tsx`) direto para `/epi/eventos` — sem
 * prefixo. `AlertsHistoryPage` fica inalcançável nessa URL. Este harness
 * passou a provar a MESMA FUNÇÃO (lista → clique → detalhe do evento) na tela
 * substituta: a lista virou `app/epi/Eventos.tsx` (exige a permissão
 * `alerts:read`, WS7 — a tela antiga não tinha esse gate) e o detalhe virou
 * `app/epi/EventoDetalhe.tsx` (`<h1>Evento #...</h1>`, não mais "Detalhe do
 * Alerta"). A navegação agora é pelo link "Abrir →" de cada linha — a linha
 * inteira não é mais clicável. Recuperação: `git show
...
async function setupRoutes(page: Page) {
  ...
  // O modal foi substituído pela página /epi/eventos/:id (EventoDetalhe) — o
  // clique agora é no link "Abrir →" da linha, que navega e busca o DETALHE.
