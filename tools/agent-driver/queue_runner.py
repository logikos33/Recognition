...
from driver import _load_spec as _load_spec_full, commit_retry_on_branch, run_implementer_retry  # noqa: E402
...
def _build_review_feedback_str(review_result: dict[str, Any], retry_n: int) -> str:
    """Monta string de feedback do revisor para prompt de retry — em memória, sem arquivo."""
    findings_lines = "\n".join(
    ...
    return (
        f"REQUEST_CHANGES — O revisor adversarial encontrou problemas (retry {retry_n}).\n"
        f"Corrija TODOS antes de finalizar:\n\n"
    ...
    for i, spec_path in enumerate(specs, start=1):
        ...
        for review_attempt in range(reviewer_max_retries + 1):
            ...
            if review_attempt >= reviewer_max_retries:
                ...
                break
            log.warning(
                "REQUEST_CHANGES: re-rodando implementador em memória (revisão %d/%d).",
                review_attempt + 1, reviewer_max_retries,
            )
            # In-memory retry — sem arquivo temporário, mesma branch agent/*
            _, spec_body = _load_spec_full(spec_path)
            feedback_str = _build_review_feedback_str(review_result, review_attempt + 1)
            eval_passed, _ = run_implementer_retry(
                spec_path,
                spec_body,
                front.get("eval", "default"),
                feedback_str,
                config,
                log,
            )
            if not eval_passed:
                log.error("Retry pós-review falhou na eval. Lote PAUSADO.")
                return EXIT_DRIVER_FAILED
            commit_msg = (
                f"fix: review retry {review_attempt + 1} — {spec_path.stem}\n\n"
                "Generated by agent-driver (D3). Human review required before merge."
            )
            if not commit_retry_on_branch(commit_msg, log):
                log.warning("Retry pós-review: nada a commitar (sem mudanças).")
            # current_pr permanece o mesmo — mesma branch agent/*, PR auto-atualizado pelo push
